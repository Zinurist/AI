...
public class Save {
	...
	/**
	 * Asks for the file path and creates an empty file. If the file already exists and overrideFile is set to true, then the user will be asked, if he wants to override it. 
	 * Set overrideFile to false, is you want to add text to the end of the file.
	 * @param saveObject a string used for in the dialog to describe the file
	 * @param defaultLoc the default location shown in the input field
	 * @param overrideFile if set to true, the program will ask to override existing files
	 * @return the path of the file
	 */
	public static String saveDialog(String saveObject,String defaultLoc,boolean overrideFile){
		Save s=new Save("");
		String path="";
		try {
			path=(String) JOptionPane.showInputDialog(null, "Where do you want to save "+ saveObject +"?", "Saving Location", JOptionPane.QUESTION_MESSAGE, null, null, defaultLoc);
			if(path!=null && !path.isEmpty()){
				...
				if(s.getFile().exists()){
					if(overrideFile){
						int opt=JOptionPane.showConfirmDialog(null, "This file already exists. Do you want to override it?", "Override File", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
			...
			JOptionPane.showConfirmDialog(null, "Error: "+e.getMessage(), "Error", JOptionPane.OK_OPTION, JOptionPane.ERROR_MESSAGE);
		}
	...
	/**
	 * Writes the given Strings seperated by the String seperator into the file. The file won't be overwritten, the strings will be written at the end of the file. (see openFileWriter)
	 * @param seperator the seperator between the given strings to write
	 */
